...
async def is_english_letters(text):
...
async def process_token(message: Message, state: fsm.FSMContext):
    ...
    if not await is_english_letters(token):
        await message.answer('Використовуйте лише англійські букви')
    else:
...
async def price_crypto(message: Message):
    ...
    await add_token_request(token_value[0].upper())
